 ...
 *
 * <p>
 *     此类管理quorum协议。 该服务器可以有三种状态：
 * <ol>
 * 领导者选举 - 每个服务器将选出一名领导者（最初提议自己为领导者）。
 * 追随者 - 服务器将与领导者同步并复制任何事务。
 * 领导者 - 服务器将处理请求并将其内容转发给追随者。 大多数粉丝必须先记录该请求才能接受该转发。
 * </ol>
 * 该类将设置一个数据报套接字，它将始终响应其当前领导者的视图。 答复将采取以下形式：
 * <pre>
 *
 *     int xid;
 *   
